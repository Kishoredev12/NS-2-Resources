 ...
 * October, 2005
 *
 * DSDV code for NS2 version 2.26 or later
 * Note: this implementation of DSDV is different from its original 
 *       version wich implemented by Brad Karp, Harvard Univ. 1999
 *       It is not guaranteed precise implementation of the DSDV design
 */

#include "DSDV_neighbor.h"
...
DSDVNeighbors::~DSDVNeighbors(){
  struct DSDV_neighbor *temp = head_;
  while(temp){
    temp = temp->next_;
    free(head_);
    head_ = temp;
...
double 
DSDVNeighbors::getdis(double ax, double ay, double bx, double by){
  double tempx = ax - bx;
  double tempy = ay - by;
...
int
DSDVNeighbors::nbsize(){
  return nbSize_;
}
// Received a verification reply
void
DSDVNeighbors::myinfo(nsaddr_t mid, double mx, double my){
  my_id_ = mid;
  my_x_ = mx;
  my_y_ = my;
}

struct DSDV_neighbor*
DSDVNeighbors::getnb(nsaddr_t nid){
  struct DSDV_neighbor *temp = head_;
  while(temp){
    if(temp->id_ == nid){
      if((DSDV_CURRENT - temp->ts_) < DEFAULT_DSDV_TIMEOUT)
	return temp;
      else {
...
void 
DSDVNeighbors::newNB(nsaddr_t nid, double nx, double ny){
  struct DSDV_neighbor *temp = getnb(nid);

  if(temp==NULL){ //it is a new neighbor
    temp=(struct DSDV_neighbor*)malloc(sizeof(struct DSDV_neighbor));
    temp->id_ = nid;
    temp->x_ = nx;
    temp->y_ = ny;
    temp->ts_ = DSDV_CURRENT;
...
void
DSDVNeighbors::delnb(nsaddr_t nid){
  struct DSDV_neighbor *temp = getnb(nid);
  if(temp==NULL) return;
  else delnb(temp);
}
...
void
DSDVNeighbors::delnb(struct DSDV_neighbor *nb){
  struct DSDV_neighbor *preffix = nb->prev_;
...
void
DSDVNeighbors::delalltimeout(){
  struct DSDV_neighbor *temp = head_;
  struct DSDV_neighbor *dd;
  while(temp){
    if((DSDV_CURRENT - temp->ts_) >= DEFAULT_DSDV_TIMEOUT){
      dd = temp;
      temp = temp->next_;
...
nsaddr_t
DSDVNeighbors::gf_nexthop(double dx, double dy){
  struct DSDV_neighbor *temp = head_;
...
struct DSDV_neighbor *
DSDVNeighbors::gg_planarize(){
    struct DSDV_neighbor *temp, *result, *index;
    index = head_;
    result = NULL;
...
double
DSDVNeighbors::angle(double x1, double y1, double x2, double y2){
 ...
 */
int
DSDVNeighbors::intersect(nsaddr_t theother, double sx, double sy,
			 double dx, double dy){
  //line 1 (x1,y1)--(x2,y2) is the segment
  //line 2 (x3,y3)--(x4,y4) is the xD 
  struct DSDV_neighbor *other = getnb(theother);
...
int
DSDVNeighbors::num_of_neighbors(struct DSDV_neighbor *nblist){
  struct DSDV_neighbor *temp = nblist;
  int counter = 0;
  while(temp){
    counter++;
    temp = temp->next_;
...
void
DSDVNeighbors::free_neighbors(struct DSDV_neighbor *nblist){
  struct DSDV_neighbor *temp, *head;
  head = nblist;
  while(head){
    temp = head;
    head = head->next_;
...
nsaddr_t
DSDVNeighbors::peri_nexthop(int type_, nsaddr_t last,
			    double sx, double sy,
			    double dx, double dy){
  struct DSDV_neighbor *planar_neighbors, *temp;
  ...
  if(last>-1){
    struct DSDV_neighbor *lastnb = getnb(last);
    if(lastnb == NULL) {
